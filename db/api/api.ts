...
export const getUserAttendance = async () => {
    const fetchedUserAttendance: userAttendanceSelectType[] = await db
        .select()
        .from(userAttendance);
...
export const getPreDefinedTT = async () => {
    const fetchedPreDefinedTT: preDefinedTimeTableSelectType[] = await db
        .select()
...
export const submitPredefinedTTForm = async ({
    degreeId,
    branchId,
    semester,
}: submitPredefinedTTFormProps) => {
    const fetchedPredefinedFormData: preDefinedTimeTableSelectType[] = await db
        .select()
        .from(preDefinedTimeTable)
        .where(
            and(
                eq(preDefinedTimeTable.degreeId, degreeId),
                eq(preDefinedTimeTable.branchId, branchId),
                eq(preDefinedTimeTable.semester, semester)
            )
        );
    ...
    fetchedPredefinedFormData?.forEach((data) => {
        if (data.isLecture) {
            promises.push(
                db!.insert(userTimeTable).values({
...
export const getAttendanceDate = async (courseId: string) => {
    const attendanceDates = await db
        .select({
            ...
            absOrPre: userAttendance.absOrPre,
            createdAt: userAttendance.createdAt,
        })
        .from(userAttendance)
        .where(eq(userAttendance.courseId, courseId))
        .orderBy(desc(userAttendance.createdAt));
...
export const undoAttendance = async (courseId: string) => {
    ...
    await db
        ...
        .where(
            and(
                eq(userAttendance.courseId, courseId),
                eq(userAttendance.createdAt, latestRow[0].createdAt)
            )
        );
};

export const editCourseId = async (data: EditCourseIdProps) => {
    const updateTT = await db
        .update(userTimeTable)
        .set({ courseId: data.newCourseId })
        .where(eq(userTimeTable.courseId, data.courseId))
        .returning();
    const updateAtt = await db
        .update(userAttendance)
        .set({ courseId: data.newCourseId })
        .where(eq(userAttendance.courseId, data.courseId))
        .returning();
